...
/* Determine maximum length of the shortest line: */
  ...
  for ( ;  w1 != head;  w1 = w1->prev) {
    ...
    if (w1->score < 0) {
      sprintf(temerr,impossibility,1);
      set_error(temerr);
      return 0;
    }
  }
  ...
  /* Determine the minimum possible longest line: */
    ...
    if (newL > L) {
      sprintf(temerr,impossibility,2);
      set_error(temerr);
      return 0;
    }
  }
...
/* between newL and the lengths of the lines:         */
  ...
  if (head->next && head->next->score < 0) {
    sprintf(temerr,impossibility,3);
    set_error(temerr);
    return 0;
  }
                ...
                int prefix, int suffix, int hang, int last, int min)
{
  ...
  char *q1, *q2, **outlines=NULL,*nullline = NULL;
...
/* Set the pointers to the suffixes, and create the words: */
  ...
  if(L==0){
    sprintf(temerr,"<width> (%d) should not equal to <prefix> (%d) + <suffix> (%d)\n",width,prefix,suffix);
    set_error(temerr);
    goto rfcleanup;
  }
  if(L<0){
    sprintf(temerr,"<width> (%d) should not less than <prefix> (%d) + <suffix> (%d)\n",width,prefix,suffix);
    set_error(temerr);
    goto rfcleanup;
  }
