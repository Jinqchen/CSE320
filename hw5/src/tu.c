...
int tu_dial(TU *tu, TU *target) {
    ...
    if(target!=NULL){
        if(tu->state!=TU_DIAL_TONE){
            ...
            if(tu->fd==target->fd){
                ...
                dprintf(tu_fileno(tu),"%s %d\n",tu_state_names[TU_BUSY_SIGNAL],tu_fileno(tu));   
            }
            else if(target->state!=TU_ON_HOOK||/*target->ref!=0*/target->peer!=NULL){
                tu->state=TU_BUSY_SIGNAL;
                dprintf(tu_fileno(tu),"%s %d\n",tu_state_names[TU_BUSY_SIGNAL],tu_fileno(tu));   
                ...
                //target->whocallme=tu;
                target->peer=tu;
                dprintf(tu_fileno(target),"%s %d\n",tu_state_names[TU_RINGING],tu_fileno(target));  
                tu->state= TU_RING_BACK;
                //tu->mecallwho=target;
                tu->peer=target;
                dprintf(tu_fileno(tu),"%s %d\n",tu_state_names[TU_RING_BACK],tu_fileno(tu)); 
    ...
    else if(target==NULL&&tu->state==TU_DIAL_TONE){
        tu->state=TU_ERROR;
        dprintf(tu_fileno(tu),"%s %d\n",tu_state_names[TU_ERROR],tu_fileno(tu)); 
        V(&mutex);  
        return -1;
        
    }
    //same as the originating TU,  TU_BUSY_SIGNAL state.
    V(&mutex);
    return 0;
...
int tu_pickup(TU *tu) {
    ...
    if(tu->state!=TU_ON_HOOK||tu->state!=TU_RINGING){
        ...
        dprintf(tu_fileno(tu),"%s %d\n",tu_state_names[TU_DIAL_TONE],tu_fileno(tu));   
    }
    if(tu->state==TU_RINGING){
        tu->state=TU_CONNECTED;
        dprintf(tu_fileno(tu),"%s %d\n",tu_state_names[TU_CONNECTED],tu_fileno(tu));  
        TU *temp=tu->peer;
        temp->state=TU_CONNECTED;
        dprintf(tu_fileno(temp),"%s %d\n",tu_state_names[TU_CONNECTED],tu_fileno(temp));  
...
int tu_hangup(TU *tu) {
    ...
    if(tu->state==TU_CONNECTED||tu->state==TU_RINGING){
        tu->state=TU_ON_HOOK;
        dprintf(tu_fileno(tu),"%s %d\n",tu_state_names[TU_ON_HOOK],tu_fileno(tu));  
        tu->peer->state=TU_DIAL_TONE;
        dprintf(tu_fileno(tu->peer),"%s %d\n",tu_state_names[TU_DIAL_TONE],tu_fileno(tu->peer)); 
        ...
        dprintf(tu_fileno(tu->peer),"%s %d\n",tu_state_names[TU_ON_HOOK],tu_fileno(tu->peer)); 
...
int tu_chat(TU *tu, char *msg) {
    ...
    dprintf(tu_fileno(tu->peer),"CHAT %s\n",msg);
    //dprintf(tu_fileno(tu),"%s %d\n",)
    V(&mutex);
    return 0;
}
